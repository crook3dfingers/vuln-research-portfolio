...
| **TAK ecosystem** (TAK Server, ATAK CIV, WinTAK, iTAK) | [`vuln-research-tak`](https://github.com/crook3dfingers/vuln-research-tak) | F-0034 draft, vendor disclosure pending | Pre-auth remote ASLR-bypass RCE chain in ATAK CIV (lab-confirmed end-to-end on x86 Android emulator). Composes a client-side native-parser code-execution primitive with an automatic crash-log address-disclosure channel. ARM64 verification pending hardware. Authenticated TAK Server findings: federation mission-package XXE (F-0032) and authenticated profile-directory traversal (F-0033). |
