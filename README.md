...
| **TAK ecosystem** (TAK Server, ATAK CIV, WinTAK, iTAK) | [`vuln-research-tak`](https://github.com/crook3dfingers/vuln-research-tak) | F-0034 draft, vendor disclosure pending | Pre-auth remote ASLR-bypass RCE chain in ATAK CIV (lab-confirmed end-to-end on x86 Android emulator against a master-branch ATAK CIV 4.6.0.5 build of the now-archived `deptofdefense/AndroidTacticalAssaultKit-CIV` repo; source-level verification of cited code paths against the current `TAK-Product-Center/atak-civ` 5.5.1.8 release; runtime re-confirmation against 5.5.1.8 and ARM64 verification pending hardware). Composes a client-side native-parser code-execution primitive with an automatic crash-log address-disclosure channel. Authenticated TAK Server findings: federation mission-package XXE (F-0032) and authenticated profile-directory traversal (F-0033). |
