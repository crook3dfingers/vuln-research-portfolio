...
| **Samsung Shannon LTE baseband + Pixel 10 GSA Trusty TEE** | [`vuln-research-baseband-shannon`](https://github.com/crook3dfingers/vuln-research-baseband-shannon) | FirmWire emulation campaign + TEE audit; Apr 2026 frankel CP1A.260405.005 stable refresh in progress | Pre-auth wireless RCE research |
| **Apple C1X cellular baseband** | _local-only `projects/apple-c1x-baseband/`_ | **Scoped** — scaffold-only, Phase 0 starting | Apple's in-house cellular modem on iPhone Air (`iPhone18,1`) and iPhone 17e; pre-auth wireless RCE via Apple Security Bounty. Intel XMM 7560 lineage; only one public CVE (CVE-2025-31214) as of project start; community IDA loader at `nlitsme/AppleC4000`; no public emulator. |
| **Apple N1 (Proxima) Wi-Fi/BT chip** | _local-only `projects/apple-n1-wireless/`_ | **Scoped** — scaffold-only, Phase 0 starting | Apple's first in-house combo Wi-Fi 7 / Bluetooth 6 / Thread wireless chip across the iPhone 17 family. Zero public CVEs, zero public RE writeups, no public format spec or loader as of project start. Highest novelty surface in the wireless-silicon landscape. |
| **Pixel 10 Dauntless security chip** | _local-only `projects/pixel-10-dauntless/`_ | **Scoped** — scaffold-only, Phase 0 starting | Google's post-Citadel security coprocessor (successor to Titan M2) on Pixel 10 / 10 Pro / 10 Pro XL / 10 Pro Fold. Near-zero public RE — Damiano Melotti's Quarkslab Citadel work does not directly transfer. Disclosure via AGDSRP / Google Bug Hunters. |
...
| **Amazon Sidewalk gateway RF** (HackerOne — `amazonvrp-devices`) | private `vuln-research-amazonvrp-sidewalk` | **Scoped** — H1 bug-bounty (access pending), Phase 0 scaffolding complete | Amazon Sidewalk-capable Echo / Ring / Blink gateways as pre-auth RF attack surface across 902–928 MHz ISM band. Endpoint-SDK fuzz harness + BladeRF delivery + 4-oracle gateway confirmation. Target class: IoT mesh-gateway parsing on commodity-RF wide-distribution devices. |
| **1Password CTF** (HackerOne — `1password_ctf`) | private `vuln-research-1password-ctf` | **Scoped** — H1 single-prize CTF ($1M), CTF tenant access pending | Capture a specific secure note inside a dedicated `bugbounty-ctf.1password.com` 1Password account. Pre-auth + post-auth research against the production 1Password.com web/sync infrastructure restricted to the researcher-owned CTF tenant. Vendor hints: Security Design White Paper §"Beware of the Leopard" (p.68) + 1Password Burp session analyzer. |
